...
"""Fork from https://github.com/kislyuk/watchtower"""
...
from watcher.unity import EC2ID
...
class CloudWatchLogHandler(handler_base_class):
    ...
    def __init__(self, log_group=sys.argv[0].split('/')[-1].split('.', 1)[0], stream_name=None, use_queues=True,
                 send_interval=60, max_batch_size=1024 * 1024, max_batch_count=10000, boto3_session=None,
    ...
    def emit(self, message):
        ...
        # 日志的消息体
        # [%(asctime)s] {} [%(processName)s] %(filename)s [line:%(lineno)d] %(levelname)s %(message)s
        msg_body = {'process': message.process, 'filename': message.filename, 'lineno': message.lineno,
                    'levelname': message.levelname, 'message': message.msg, 'args': message.args, 'instanceID': EC2ID}
        message.msg = json.dumps(msg_body)
